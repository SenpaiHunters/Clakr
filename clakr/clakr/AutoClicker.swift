...
class AutoClicker: ObservableObject {
    ...
    var stationarySeconds: TimeInterval = 3
        ...
        func startClicking(clicksPerSecond: Double, startAfter: TimeInterval, stopAfter: TimeInterval, stationaryFor: TimeInterval) {
...
private func startTimer() {
    ...
    guard isClicking, Date().timeIntervalSince(lastMoved) >= stationarySeconds else {
        print("Click skipped: isClicking=\(isClicking), timeIntervalSinceLastMoved=\(Date().timeIntervalSince(lastMoved))")
        return
    }
    ...
    guard let mouseDownEvent = CGEvent(mouseEventSource: nil, mouseType: .leftMouseDown, mouseCursorPosition: point, mouseButton: .left),
          let mouseUpEvent = CGEvent(mouseEventSource: nil, mouseType: .leftMouseUp, mouseCursorPosition: point, mouseButton: .left) else {
